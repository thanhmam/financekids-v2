...
export default function ExplorePage() {
  ...
  function getTopicProgress(topic) {
    ...
    const total = LESSONS.filter(l => topic.lessonIds.includes(l.id)).length || topic.lessonIds.length || 1;
        ...
        <div style={{
          ...
          <input
            ...
            style={{
              ...
              font: "600 13px 'Nunito'", color: "#15392A",
            }}
          />
        </div>
          ...
          {filtered.map(topic => {
            ...
            return (
              <button
                ...
                className="btn-press"
                onClick={() => {
                  if (!topic.locked && topic.lessonIds[0]) {
                    router.push(`/game/${topic.lessonIds[0]}`);
                  }
                }}
                style={{
                  background: topic.bg, borderRadius: 18, padding: 14,
                  border: "none", cursor: topic.locked ? "default" : "pointer",
                  textAlign: "left", position: "relative",
                }}
              >
                <div style={{ color: topic.iconColor, font: "800 22px 'Baloo 2'" }}>{topic.icon}</div>
                <div style={{ font: "800 14px 'Baloo 2'", color: topic.titleColor, marginTop: 8 }}>{topic.label}</div>
                <div style={{ font: "600 11px 'Nunito'", color: topic.subColor, marginTop: 2 }}>
                  {topic.locked
                    ? (topic.lockCost ? `Khoá · ${topic.lockCost}` : "Khoá")
                    : allDone
                    ? `${progress.total}/${progress.total} bài · Xong`
                ...
                {allDone && (
                  <div style={{
                    ...
                    background: "#16C172", display: "flex", alignItems: "center",
                    justifyContent: "center", color: "#fff", font: "800 11px 'Baloo 2'",
                  }}>✓</div>
                )}
              </button>
            );
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: "#fff", border: `2px solid ${done ? "#16C172" : "#ECF1E6"}`,
              ...
              >
                ...
                {done && (
                  <div style={{ width: 24, height: 24, borderRadius: 8, background: "#16C172", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", font: "800 11px 'Baloo 2'", flexShrink: 0 }}>✓</div>
                )}
              </button>
            );
          })}
