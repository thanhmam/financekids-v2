...
- **Luồng** (4 bước): intro (giới thiệu + ưu điểm) → trả lời **3 câu hỏi** (mục tiêu / trình độ / nhịp học) → animation "XuXu đang phân tích…" → **lộ trình gợi ý**: danh sách bài học (`LessonCard`) lọc & sắp xếp theo chủ đề + cấp độ đã chọn, kèm câu tóm tắt cá nhân hóa.
