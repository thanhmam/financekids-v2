...
- **Nhật ký cập nhật / Change Log** (`/changelog`): timeline thay đổi, tag **Mới / Cải thiện / Sửa lỗi**. **Tự đọc trực tiếp từ [../CHANGELOG.md](../CHANGELOG.md)** (SSOT) — chỉ rút gọn thay đổi chính, bỏ phần header quy tắc/ghi chú. Không có dữ liệu changelog riêng.
